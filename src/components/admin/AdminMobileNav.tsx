    ...
    <>
      ...
      {open ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          />
          <aside className="relative h-full w-[min(78vw,280px)] overflow-y-auto bg-[#243447] text-white shadow-2xl">
            ...
            <nav className="grid gap-4 px-3 py-4">
              {groups.map((group) => (
                <div key={group.label}>
                  <p className="mb-1 px-2 text-[10px] font-black uppercase tracking-wide text-sky-200/75">{group.label}</p>
                    ...
                    {group.links.map((link) => {
                      ...
                      return (
                        <Link
                          ...
                          href={link.href}
                          onClick={() => setOpen(false)}
                          className="flex items-center gap-2.5 rounded-md px-2.5 py-2 text-[12px] font-bold text-white/86 hover:bg-[#4285f4] hover:text-white"
                        >
